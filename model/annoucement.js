...
module.exports.createAnnoucement = function(annoucement, cb)
{
	pg.connect(connString, function(err, client, done) {
		...
		client.query("INSERT INTO \"Annoucement\"(title, description, owner, category,  locate, state) VALUES($1, $2, $3, $4, $5, $6)",
			[annoucement.title, annoucement.description, annoucement.owner, annoucement.category, annoucement.locate, annoucement.state],
			function(err, result)
			{
				done();
				if(err) return cb(err);
...
module.exports.getIdAnnoucement = function(id, cb)
{
	pg.connect(connString, function(err, client, done) {
		...
		client.query("SELECT annouce_id, title, description, owner, category, locate, state FROM \"Annoucement\" where annouce_id = $1",
			[id],
			function(err, result)
			{
				done();
				...
				var annouce = new Annoucement(result.rows[0].id, result.rows[0].title, result.rows[0].description, result.rows[0].owner, result.rows[0].category, result.rows[0].locate, result.rows[0].state);
				cb(null, annouce);
			}
		);
	});
